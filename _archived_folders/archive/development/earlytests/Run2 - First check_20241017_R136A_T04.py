...
working_directory = working_directory + "/R136T04"
...
File_type = "BIAS"
File_NR = '074'
file = os.path.join(data_directory_16,'bias.'+File_NR+'.fits')
hdu = read_SAMI_mosaic(file)
Bias = hdu.data
hdu.data = Bias
hdul = fits.HDUList([hdu]) ; hdul.writeto(File_type+'.fits',overwrite=True)
...
#QUARTZ IMAGE
File_type = "QUARTZ_NOSPECTRA"
file  = os.path.join(data_directory_16,	'target.053.fits')
hdu = read_SAMI_mosaic(file)
Quartz_NoSpectra = hdu.data
hdu.data = Quartz_NoSpectra
hdul = fits.HDUList([hdu]) ; hdul.writeto('Quartz_NoSpectra.fits',overwrite=True)

#TARGET
File_type = "TARGET"
file  = os.path.join(data_directory_17,	'target.042.fits')
hdu = read_SAMI_mosaic(file)
Target = hdu.data
hdu.data = Target
hdul = fits.HDUList([hdu]) ; hdul.writeto('Target.fits',overwrite=True)


#plt.imshow(mask, vmin=1500, vmax=18000)
Quartz_Spectra_Bias = Quartz_Spectra - Bias
Quartz_NoSpectra_Bias= Quartz_NoSpectra - Bias
Target_Bias = Quartz_NoSpectra - Bias


plt.imshow(Quartz_Spectra_Bias-Quartz_NoSpectra_Bias*0.8, vmin=0, vmax=500)
plt.show()
