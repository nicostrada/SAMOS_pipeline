...
class WCS:
    ...
    def read_GSP_wcs(self, ccd):
        ...
        """
        assert isinstance(ccd, CCDData)
        self.model_name = model_name = ccd.header['GSP_FUNC']
        
        self.degree = degree = ccd.header['GSP_ORDR']
        self._binning = int(ccd.header['CCDSUM'].split()[0])

        if model_name == 'Chebyshev1D':
            self.model = model = models.Chebyshev1D(degree=self.degree)
            print(type(model),type(self.model))

            for i in range(int(ccd.header['GSP_ORDR']) + 1):
                model.__getattribute__('c{:d}'.format(i)).value = ccd.header[
                    'GSP_C{:03d}'.format(i)]
            self.wavelength_and_intensity = [
                model(range(ccd.header['GSP_NPIX'])), ccd.data]
