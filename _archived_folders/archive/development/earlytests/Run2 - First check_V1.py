...
#check
    ...
    trace[y0:y1,:]=1
    hdu.data = trace
    hdul = fits.HDUList([hdu]) ; hdul.writeto('/Users/robberto/Desktop/trace_'+str(i_mask+1)+'.fits',overwrite=True)
...
File_NR = '088'
files = []
files.append('target.'+File_NR+'.fits')
hdu = read_SAMI_mosaic(files[0])
all_data = hdu.data
...
File_NR = '074'
files = []
files.append('bias.'+File_NR+'.fits')
hdu = read_SAMI_mosaic(files[0])
bias = hdu.data
...
#EXTRACT FRAMES WITH THE SINGLE SPECTRA
for i_mask in range(Nr_of_masks):
#    plt.imshow(all_data_biassub,vmin=0,vmax=200)
#    plt.show()
    hdul = fits.open('/Users/robberto/Desktop/trace_'+str(i_mask+1)+'.fits')
    trace =   hdul[0].data
...
#    plt.show()
    ...
    hdul = fits.HDUList([hdu]) ; hdul.writeto('/Users/robberto/Desktop/data_masked_'+str(i_mask+1)+'.fits',overwrite=True)
...
#    plt.show()
    ...
    hdul = fits.HDUList([hdu]) ; hdul.writeto('/Users/robberto/Desktop/flat_masked_'+str(i_mask+1)+'.fits',overwrite=True)
    print('done mask',i_mask)
    ...
    hdu.data = spec2d_ff
    hdul = fits.HDUList([hdu]) ; hdul.writeto('/Users/robberto/Desktop/spec2d_ff_'+str(i_mask+1)+'.fits',overwrite=True)
...
#    plt.show()
    ...
    axes[counter].plot(spec1d_ff)
    axes[counter].set_title(f'Slit {i_mask}' )
    ymax = min((heapq.nlargest(50, spec1d_ff)))*2
    axes[counter].axis(ymin=0, ymax=ymax)#np.median(spec1d_ff)*3)  # Sets the y-axis range from 0 to 1
    #plt.subplot(10,1,counter+1)
    #plt.plot(spec1d_ff)
    if counter == 9:
        plt.tight_layout()
        plt.savefig('/Users/robberto/Desktop/spectra_ff_'+str(plot_nr)+'.')        
        plt.show()
        fix, axes = plt.subplots(nrows=10, ncols=1, figsize = (12,20))
